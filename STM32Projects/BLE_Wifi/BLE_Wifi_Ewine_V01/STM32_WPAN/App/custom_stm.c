...
typedef struct{
  ...
  uint16_t  CustomGssidHdle;                  /**< get_ssid handle */
  uint16_t  CustomGpassHdle;                  /**< get_password handle */
  uint16_t  CustomAckHdle;                  /**< status_connection handle */
}CustomContext_t;
...
/* Private variables ---------------------------------------------------------*/
uint8_t SizeGssid = 30;
uint8_t SizeGpass = 30;
uint8_t SizeAck = 1;
...
#define COPY_GET_SSID_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xab,0xcd,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_GET_PASSWORD_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x12,0x34,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_STATUS_CONNECTION_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xde,0xad,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomGpassHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_GPASS:
      ret = aci_gatt_update_char_value(CustomContext.CustomFsHdle,
                                       CustomContext.CustomGpassHdle,
                                       0, /* charValOffset */
                                       SizeGpass, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
